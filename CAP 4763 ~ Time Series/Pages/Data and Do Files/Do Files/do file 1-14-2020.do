...
*Why are the ts lines so bumpy?

*Detrend and add quarterly indicators to deal with seasonality
...
reg lnreratio lnrgdppc date i.quarter, robust

*Problem, can't really interpret standard errors yet,
**We have not yet dealt with serial correlation!
***Lack of independence between residuals has to be dealt with for inference

*Won't get past here on 1-14-2020
...
*model from last time was static models:
reg lnreratio lnrgdppc i.q date if tin(1959q3,2017q4)

*Do things change over time?
**Maybe focus on a more recent period?
reg lnreratio lnrgdppc i.q date if tin(1980q1,2017q4) 

*Add a lagged value of x to get a distrbuted lag model. WHY????
reg lnreratio lnrgdppc l.lnrgdppc i.q date if tin(1980q1,2017q4)
*how many lags???
reg lnreratio lnrgdp l.lnrgdp l2.lnrgdp i.q date if tin(1980q1,2017q4)
*a shorter way to code lags
reg lnreratio l(0/4).lnrgdp i.q date if tin(1980q1,2017q4)
reg lnreratio l(0/8).lnrgdp i.q date if tin(1980q1,2017q4)
reg lnreratio l(0/12).lnrgdp i.quarter date if tin(1980q1,2017q4)

***Huh????????


*start of class 1-22-2019

****NOTE: Run I(1) simulation before starting back with this 


*recall this picture
twoway (tsline reratio) (tsline r_gdp_pc, yaxis(2)) if year>=1960
* in logs
twoway (tsline lnreratio) (tsline lnrgdppc, yaxis(2)) if year>=1960
*with time trends shown
twoway (tsline lnreratio) (lfit lnreratio date) ///
 (tsline lnrgdppc, yaxis(2)) (lfit lnrgdppc date, yaxis(2)) if year>=1960
*with time trends removed from the series
twoway (tsline reslnreratio, ///
	ytitle("Log Federal Receipts to Expenditure Ratio")) ///
	(tsline reslnrgdppc, yaxis(2)  ) if year>=1960 
...
reg lnreratio l.lnreratio if tin(1980q1,2017q4)
*Above was AR(1). Next is AR(4)
**Note how lags are entered here
reg lnreratio l(1/4).lnreratio if tin(1980q1,2017q4)
*Can keep going...
reg lnreratio l(1/8).lnreratio if tin(1980q1,2017q4)
reg lnreratio l(1/12).lnreratio if tin(1980q1,2017q4)
...
*How to test if I(1)?
reg d.lnreratio l.lnreratio date 
*"Theta" is less than one, implies not I(1)
*But: t-dist not right is it is I(1), so...

*Dickey Fuller Test
dfuller lnreratio, trend regress
dfuller lnreratio, trend lags(4) regress
*Appears not strictly I(1). BUT...
*Even if not I(1), STILL rho is 0.94, pretty long lasting impacts
**Good idea to work with first differenced data


*Instead of just AR Models, let us consider some ARDL Models:
reg d.lnreratio l(0/4)d.lnrgdp i.q date if tin(1980q1,2017q4)
reg d.lnreratio l(1/4)d.lnreratio l(0/4)d.lnrgdp i.q date if tin(1980q1,2017q4)
reg d.lnreratio l(1/4)d.lnreratio l(0/1)d.lnrgdp i.q date if tin(1980q1,2017q4)
...
pac res
*Might be OK here. How to check formally?
 
**In the spirit of:
reg res l(1/12).res l(0/1)d.lnrgdp i.q date if tin(1980q1,2017q4)
testparm l(1/12).res
...
*Does not appear to have remaining serial correlation
...
*No evidence remaining of serial correlation
